...
Powodów takich rezultatów może być kilka. Pierwszego z nich należy upatrywać się w samych danych. Bardzo istontnym założeniem czynionym przy modelowaniu z wykorzystaniem Sieci Bayesa, jest niezależność zmiennych losowych. W przypadku gdy to założenie nie jest spełnione, Sieci Bayesa nie uzyskują zadowalających rezultatów. Ponadto w opisie zbioru można znaleźć: "(dla odmiedniczkowego zapalenia nerek) Symptomy ostrego odmiedniczkowego zapalenia pęcherza moczowego występują bardzo często". Może to stanowić poważną trudność dla klasyfikatora opartego o reguły probablistyczne. 
