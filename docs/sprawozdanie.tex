...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[main node] (Cone) {$C_1$};
  \node[main node] (Ctwo) [below of=Cone] {$C_2$};
  \node[main node] (Cn) [below of=Ctwo] {$C_n$};
  \node[main node] (E) [right of=Ctwo] {$E$};

  \path[every node/.style={font=\sffamily\small,
  		fill=white,inner sep=1pt}]
  	% Right-hand-side arrows rendered from top to bottom to
  	% achieve proper rendering of labels over arrows.
    (Cone) edge node[right=1mm] {$p_1$} (E)
    (Ctwo) edge node[right=1mm] {$p_2$} (E)
    (Cn) edge node[right=1mm]  {$p_n$} (E);
\end{tikzpicture}
