...
def test_joint_distribution_sums_up_to_1():
    x_data = np.random.randint(0, 2, size=(100, 8))
    y_data = np.random.randint(0, 2, size=(100, 3))
    sut = BayesianNetwork()
    sut.fit(x_data, y_data)
    joint_sum = np.sum(sut.joint_prob, axis=None)
    assert np.isclose(joint_sum, 1.0)


def test_each_elem_of_joint_is_between_0_and_1():
    x_data = np.random.randint(0, 2, size=(100, 8))
    y_data = np.random.randint(0, 2, size=(100, 3))
    sut = BayesianNetwork()
    sut.fit(x_data, y_data)

    assert_is_probability(sut.joint_prob)
...
def test_test_case2():
    ...
    assert network.joint_prob[0, 0, 0] == 0.05
    assert network.joint_prob[0, 0, 1] == 0.1
    assert network.joint_prob[0, 1, 0] == 0.05
    assert network.joint_prob[0, 1, 1] == 0.2
    assert network.joint_prob[1, 0, 0] == 0.2
    assert network.joint_prob[1, 0, 1] == 0.25
    assert network.joint_prob[1, 1, 0] == 0.1
    assert network.joint_prob[1, 1, 1] == 0.05
...
def test_test_case3():
    ...
    assert network.joint_prob[0, 0, 0, 0] == 0.05
    assert network.joint_prob[0, 0, 0, 1] == 0.01
    assert network.joint_prob[0, 0, 1, 0] == 0.02
    assert network.joint_prob[0, 0, 1, 1] == 0.1
    assert network.joint_prob[0, 1, 0, 0] == 0.02
    assert network.joint_prob[0, 1, 0, 1] == 0.02
    assert network.joint_prob[0, 1, 1, 0] == 0.03
    assert network.joint_prob[0, 1, 1, 1] == 0.05
    assert network.joint_prob[1, 0, 0, 0] == 0.06
    assert network.joint_prob[1, 0, 0, 1] == 0.04
    assert network.joint_prob[1, 0, 1, 0] == 0.1
    assert network.joint_prob[1, 0, 1, 1] == 0.05
    assert network.joint_prob[1, 1, 0, 0] == 0.05
    assert network.joint_prob[1, 1, 0, 1] == 0.2
    assert network.joint_prob[1, 1, 1, 0] == 0.1
    assert network.joint_prob[1, 1, 1, 1] == 0.1
